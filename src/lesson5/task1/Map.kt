...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) {
    for (startOfRecord in b.keys)
        if (a[startOfRecord] == b[startOfRecord]) a.remove(startOfRecord)
}
...
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> {
    ...
    for (aNames in a)
        if (b.contains(aNames) && !names.contains(aNames)) names.add(aNames)
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    newMap.putAll(mapA)
    newMap.putAll(mapB)
    for ((aTitle, aPhone) in mapA)
        if (mapB.contains(aTitle) && mapB[aTitle] != mapA[aTitle]) newMap[aTitle] = aPhone + ", " + mapB[aTitle]
    return newMap
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    val numberAndIndex = mutableMapOf<Int, Int>()
    for (i in list.indices) {
        val index = numberAndIndex[number - list[i]]
        if (index != null) return Pair(index, i)
        numberAndIndex[list[i]] = i
    }
    return Pair(-1, -1)
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    for (i in treasures.size downTo 1) {
        if (maxCost[i][j] != maxCost[i - 1][j]) {
            ...
            j -= treasures.getValue(listName[i - 1]).first
        }
    }
    return result
}
